...
// 读取v2 staging数据
const staging = JSON.parse(fs.readFileSync(path.join(DATA_DIR, 'staging-products-v2.json'), 'utf8'));
...
const brandStats = {};
products.forEach(p => {
  if (!brandStats[p.brandSlug]) {
    brandStats[p.brandSlug] = { name: p.brandName, slug: p.brandSlug, count: 0 };
  }
  brandStats[p.brandSlug].count++;
});
...
fs.writeFileSync(path.join(STOREFRONT_DATA_DIR, 'brands.json'), JSON.stringify(brands, null, 2));
